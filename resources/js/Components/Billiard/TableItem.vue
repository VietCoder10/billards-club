...
const statusColor = computed(() => {
  return props.table.status === 'playing' ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600';
});

const statusText = computed(() => {
  return props.table.status === 'playing' ? 'Đang chơi' : 'Bàn trống';
});
</script>
...
<template>
  <div @click="emit('click', table)" :class="['aspect-square rounded-xl shadow-lg cursor-pointer transition-all duration-300 transform hover:scale-105 flex flex-col items-center justify-center text-white p-4', statusColor]">
    ...
    <div class="text-xl font-bold uppercase">{{ table.name }}</div>
    <div class="text-sm opacity-90 mt-1 font-medium">{{ statusText }}</div>
    <div v-if="table.status === 'playing'" class="mt-2 flex items-center gap-1 text-xs">
