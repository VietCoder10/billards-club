  ...
  <Dialog v-model:visible="localVisible" modal header="Thêm khách hàng" :dismissableMask="true" :style="{ width: '40vw' }" :breakpoints="{ '960px': '60vw', '640px': '90vw' }">
    <VeeForm as="div" v-slot="{ handleSubmit }">
      <form @submit="handleSubmit($event, onSubmit)" class="space-y-6 py-4">
        ...
        <div class="field">
          <Field name="email" rules="required|email|max:255" v-model="state.model.email" v-slot="{ field, meta, handleChange }">
            <FloatLabel variant="on">
              <InputText id="add_email" class="w-full" :modelValue="field.value" @update:modelValue="handleChange" :class="{ 'p-invalid': !meta.valid && meta.touched }" />
