      ...
      <form @submit="handleSubmit($event, onSubmit)">
        <div class="mb-4">
          ...
          <Field name="name" rules="required|max:255" v-model="state.model.name" v-slot="{ field, meta, handleChange }">
            <InputText v-model="state.model.name" v-bind="field" :class="{ 'p-invalid': !meta.valid && meta.touched, 'w-full bg-zinc-800/50 border-zinc-700 text-white placeholder:text-zinc-500 focus:border-amber-500 focus:ring-amber-500': true }" @update:model-value="handleChange" placeholder="Nguyễn Văn A" />
          </Field>
        ...
        <div class="mb-4">
          ...
          <Field name="password" rules="required|max:100|min:8" v-model="state.model.password" v-slot="{ field, meta, handleChange }">
            <Password v-model="state.model.password" v-bind="field" inputClass="w-full bg-zinc-800/50 border-zinc-700 text-white placeholder:text-zinc-500 focus:border-amber-500 focus:ring-amber-500" :class="{ 'p-invalid': !meta.valid && meta.touched, 'w-full': true }" placeholder="••••••••" hideIcon="pi pi-eye text-zinc-400" showIcon="pi pi-eye-slash text-zinc-400" :feedback="false" v-on:update:model-value="handleChange" toggleMask />
          </Field>
        ...
        <div class="mb-6">
          ...
          <Field name="password_confirmation" rules="required|confirmed:@password" v-model="state.model.password_confirmation" v-slot="{ field, meta, handleChange }">
            <Password v-model="state.model.password_confirmation" v-bind="field" inputClass="w-full bg-zinc-800/50 border-zinc-700 text-white placeholder:text-zinc-500 focus:border-amber-500 focus:ring-amber-500" :class="{ 'p-invalid': !meta.valid && meta.touched, 'w-full': true }" placeholder="••••••••" hideIcon="pi pi-eye text-zinc-400" showIcon="pi pi-eye-slash text-zinc-400" :feedback="false" v-on:update:model-value="handleChange" toggleMask />
          </Field>
